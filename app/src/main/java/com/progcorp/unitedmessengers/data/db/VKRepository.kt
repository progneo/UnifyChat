...
class VKRepository @Inject constructor(private val remoteDataSource: VKRemoteDataSource) {

    suspend fun getConversations(offset: Int): Flow<ArrayList<Conversation>> {
        return flow {
            val response = remoteDataSource.getConversations(offset)
    ...
    suspend fun getConversationById(id: Int): Flow<Conversation?>  {
        return flow {
            val response = remoteDataSource.getConversationById(id)
    ...
    suspend fun getMessages(chat: Conversation, offset: Int, count: Int): Flow<ArrayList<Message>> {
        return flow {
            val response = remoteDataSource.getMessages(chat, offset, count)
            if (response.status == ApiStatus.SUCCESS) {
    ...
    suspend fun getUsers(): Flow<ArrayList<User>> {
        return flow {
